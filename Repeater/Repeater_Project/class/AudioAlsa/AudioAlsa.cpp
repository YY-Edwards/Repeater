...
void AudioAlsa::get_record_buf(void *buff)
{
	...
	int err = 0;
		...
		if (err == -EPIPE){
			...
			if ((err = snd_pcm_prepare(capture_handle)) < 0){
				//fprintf(stderr, "cannot prepare audio interface for  use (%s) \n", snd_strerror(err));
				syslog(LOG_LOCAL7 | LOG_NOTICE, "cannot prepare audio interface for  use (%s) \n", snd_strerror(err));
			}
		}
		else if (err == -EBADFD){
			//fprintf(stderr, "PCM is not in the right state (%s)\n", snd_strerror(err));
			syslog(LOG_LOCAL7 | LOG_NOTICE, "PCM is not in the right state (%s)\n", snd_strerror(err));
		}
		else if (err == -ESTRPIPE){
			//fprintf(stderr, "a suspend event occurred (%s)\n", snd_strerror(err));
			syslog(LOG_LOCAL7 | LOG_NOTICE, "a suspend event occurred (%s)\n", snd_strerror(err));
		}
		else{
...
int8_t AudioAlsa::send_buf_playback(void *inputbuf)
{
	...
	if (err == -EPIPE){
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "write:EBADFD  failed (%s)\n", snd_strerror(err));
		...
		switch (err){
			...
			case SND_PCM_STATE_SETUP:
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "pcm state is : SNDRV_PCM_STATE_SETUP\n");
			...
			case SND_PCM_STATE_SUSPENDED:
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "pcm state is : SNDRV_PCM_STATE_SUSPENDED\n");
			...
			default:
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "unkonow err \n");
				break;
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "write:ESTRPIPE failed (%s)\n", snd_strerror(err));
			while ((err = snd_pcm_resume(playback_handle)) == -EAGAIN)
				sleep(1);
			if (err < 0){
				err = snd_pcm_prepare(playback_handle);
				if (err < 0){
					syslog(LOG_LOCAL7 | LOG_NOTICE, "can not recovery\n");
				}
