...
MyRepeater::~MyRepeater()
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "Exit Repeater\n");
...
void MyRepeater::Start()
{
	...
	if (basedevice_ID){//master
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Repeater ID is : Master \n");
	}
	else{//slave
		//fprintf(stderr, "Repeater ID is : Slave\r\n");
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Repeater ID is : Slave \n");
	}
	...
	syslog(LOG_LOCAL7 | LOG_NOTICE, "reboot the system \n");
...
void MyRepeater::config_hw(uint32_t baseip, uint32_t mulcastip)
{
	...
	if (fd != -1){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "open delay_config okay! \n");
		ret = lseek(fd, 0, SEEK_SET);
		if (ret <0)syslog(LOG_LOCAL7 | LOG_DEBUG, "lseek delay_config err! \n");//fprintf(stderr, "lseek delay_config err \n");
		ret = read(fd, databuf, 4);
		if (ret <0)syslog(LOG_LOCAL7 | LOG_DEBUG, "open delay_config okay! \n");//fprintf(stderr, "open delay_config okay \n");
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "open delay_config err \n");
	...
	uint32_t baseip, uint16_t baseport)
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "RTP init successfully...\n");
...
void MyRepeater::ProcessRTPPacket(const RTPSourceData &srcdat, const RTPPacket &rtppack)
{
	...
	if (length != 20)syslog(LOG_LOCAL7 | LOG_NOTICE, "length: %d\n", length);//fprintf(stderr, "length: %d\n", length);
	...
	if (counter > 139){
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, " RTP-recv 139p from SSRC:0x%x, tip:%d\n", current_ssrc, s_counter);
	}
...
void MyRepeater::DecodeThreadFunc()
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "DecodeThread is running...\n");
	while (1)
	{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit Decode_thread \n");
...
void MyRepeater::MasterOnDataFunc(int command, ResponeData data)
{
	...
	case SLAVEMAP:
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "RepeaterRecvmap\n");
	...
	case GETCHANNELSTATUS:
		...
		case WAIT_SLAVE_GET_CHANNEL:
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "RepeaterRecvGetChannelStatus\n");
			...
			if (channel_busy_flag){
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "Slave:%s apply for channel\n", data.getChannelIp.c_str());
				syslog(LOG_LOCAL7 | LOG_NOTICE, "Sorry, The current channel is busy...\n");
				status = WAIT_SLAVE_GET_CHANNEL;
			}
			else{
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "Slave:%s will send RTP data..\n", data.getChannelIp.c_str());
	...
	case RELEASECHANNELSTATUS:
		...
		//	fprintf(stderr, "Slave:%s has end RTP data..\n", data.releaseChannelIp.c_str());
			syslog(LOG_LOCAL7 | LOG_NOTICE, "RepeaterRecvReleaseChannel\n");
			syslog(LOG_LOCAL7 | LOG_NOTICE, "Slave:%s has end RTP data..\n", data.releaseChannelIp.c_str());
	...
	default:
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "!!!Should not happen cmd: %d\n", command);
...
void MyRepeater::SlaveOnDataFunc(int command, ResponeData data)
{
	...
	case SLAVEMAP:
		...
		//fprintf(stderr, "save map-file is finished\n");		
		syslog(LOG_LOCAL7 | LOG_NOTICE, "save map-file is finished\n");
	...
	case SETCHANNELSTATUS:
		...
		if (data.status){//busy
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "channel is busy...\n");
			syslog(LOG_LOCAL7 | LOG_NOTICE, "ignore local audio data...\n");
			...
			if (command == LOCALSETCHANNELSTATUS){
				...
				if (Mulcast_Trigger == 1){
					...
					syslog(LOG_LOCAL7 | LOG_NOTICE, "Slave RTP is Recv...\n");
					break;
				}//salve处于正在接收状态，CD触发，主机无响应，本地转发响应，则应该忽略
				//fprintf(stderr, "Master is not work!!!...\n");
				syslog(LOG_LOCAL7 | LOG_NOTICE, "Master is not work!!!...\n");
	...
	case BEGINRECORDERVOICE:
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Remote Voice begin\n");
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Some one will send RTP data..\n");
	...
	case ENDRECORDERVOICE:
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Remote Voice end\n");
...
void MyRepeater::repeater_task_start()
{
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "audio_record_pthread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "audio_playback_pthread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "mulcastport_poll_pthread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "rtp_send_pthread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "TimePollThread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "EncodeThread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "DecodeThread  create fail... \n");
	}
	...
	if (err != 0){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "CD_poll_thread  create fail... \n");
	}
...
void MyRepeater::CDPollThreadFunc()
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "CD poll thread is running...\n");
	...
	while (1)
	{
		...
		if (rc < 0){
			...
			syslog(LOG_LOCAL7 | LOG_DEBUG, "poll() failed!\n");
			break;
		}
		if (rc == 0){
		...
		if (fdset->revents & POLLPRI){//The POLLPRI event happened.
			...
			if (rc == -1)syslog(LOG_LOCAL7 | LOG_DEBUG, "lseek err\n");//fprintf(stderr, "lseek err\n");

			len = read(CD_fd, buf, 10);
			if (len == -1){
				syslog(LOG_LOCAL7 | LOG_DEBUG, "read failed\n");//fprintf(stderr, "read failed\n");
				break;
			}
			buf[1] = '\0';
			...
			if ('1' == buf[0]){//rising trriger
				...
				if (temp == HIGH_LEVEL){
					...
					syslog(LOG_LOCAL7 | LOG_NOTICE, "!!!CD_release...\n");
				...
				if (temp == LOW_LEVEL){
					...
					if (basedevice_ID == true){//master
						...
						if (channel_busy_flag == 0){
							...
							syslog(LOG_LOCAL7 | LOG_NOTICE, "channel is busy...\n");
							syslog(LOG_LOCAL7 | LOG_NOTICE, "rtp-recv is running...\n");
						}
						pthread_mutex_unlock(&flag_mutex);//unlock-flag
					}
					else//slave
					{
						...
						syslog(LOG_LOCAL7 | LOG_NOTICE, "!!!Slave CD_Trigger...\n");
						proto->GetStatus();
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit CD_poll_thread \n");
...
void MyRepeater::RecordThreadFunc()
{
	...
	if (NULL == (capture_buffer = (char *)malloc(size))){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "capture_buffer malloc error..\n");
		exit(1);
	}
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit record_pthread \n");
...
void MyRepeater::EncodeThreadFunc()
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "EncodeThread is running...\n");
	while (1)
	{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "EncodeThread is exit...\n");
...
void MyRepeater::PlaybackThreadFunc()
{
	...
	if (NULL == (playback_buffer = (char *)malloc(size))){
		...
		syslog(LOG_LOCAL7 | LOG_DEBUG, "playback_buffer malloc error..\n");
		exit(1);
	}
	...
	for (;;){
			...
			if (temp == 0){
				...
				if (play_counter > 139){
					...
					syslog(LOG_LOCAL7 | LOG_NOTICE, " local-playback 139p, tip:%d\n", s_counter);
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "m_PlayBackQueue.TakeFromQueue err : %d\n", temp);
				break;
			}
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit playback_thread \n");
	pthread_exit(NULL);
...
void MyRepeater::RTPsendThreadFunc()
{
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "Rtp send pthread is running...\n");
		...
		if (temp == 0){
			...
			for (it = sessionmap.begin(); it != sessionmap.end(); it++)
			{
				...
				if (send_counter > 139){
					...
					syslog(LOG_LOCAL7 | LOG_DEBUG, "send-RTP 139p to %s, tip:%d\n", key.c_str(), s_counter);
				}
			}
			pthread_mutex_unlock(&map_mutex);//unlock-flag
			usleep(200);//1ms
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit pthread :rtp_send_pthread\n");
...
void MyRepeater::MulcastPortPollThreadFunc()
{
	...
	while (1){
			...
			if (audio_codec_err_counter > 30){
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "Sound card failure, ready to restart the gateway\n");
			}
			if ((end.tv_sec - start.tv_sec) > 60){

				//fprintf(stderr, "remote rtp timeout!!!\r\n");
				syslog(LOG_LOCAL7 | LOG_NOTICE, "remote rtp timeout!!!\r\n");
				timeout_flag = 1;
				pthread_mutex_lock(&flag_mutex);//lock-flag
	...
	syslog(LOG_LOCAL7 | LOG_DEBUG, "exit pthread :mulcastport_poll_pthread\n");
	...
	while (1)
	{
		...
		if (((end.tv_sec - start.tv_sec) > 55) || (cd_level == HIGH_LEVEL)){
			...
			if (cd_level == LOW_LEVEL){
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "end_time:%ld , %ld ", end.tv_sec, end.tv_usec);
				timeout_flag = 1;
			}
			else
			{	
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "!!!NOTE:user has released the CD\n ");
...
void MyRepeater::rtp_session_create(std::map<std::string, std::string> slavemap)
{
	...
	syslog(LOG_LOCAL7 | LOG_NOTICE, "slavemap.size() :%d\n", size);
	...
	for (it = slavemap.begin(); it != slavemap.end(); it++)
	{
		...
		if (itSession != sessionmap.end())
		{
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "IPaddress is have\n");
			continue;
		}
		else
		{
			...
			if (err != 0){//不发给自己
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "new session create okay!!!\n");
	...
	if (basedevice_ID == false)//slave
	{
		...
		if (itSession != sessionmap.end())
		{
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "Master IPaddress is have\n");
		...
		else
		{
			...
			syslog(LOG_LOCAL7 | LOG_NOTICE, "Slave connect master session create okay!!!\n");
		}
	...
	if (slavemap.size() < sessionmap.size()){//Someone dropped
		...
		for (itSession = sessionmap.begin(); itSession != sessionmap.end(); itSession++)
		{
			...
			if (temp == slavemap.end())
			{
				...
				syslog(LOG_LOCAL7 | LOG_NOTICE, "%s has dropped...\r\n", session_key.c_str());
...
void MyRepeater::Wait_TimePoll_Event()
{
	...
	while (CD_Trigger == 0){
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Timekeeper   is ready\n");
		pthread_cond_wait(&CD_trigger_cond, &CD_cond_mutex);
		//fprintf(stderr, "Timekeeper   is running\n");
		syslog(LOG_LOCAL7 | LOG_NOTICE, "Timekeeper   is running\n");
	}
	pthread_mutex_unlock(&CD_cond_mutex);
...
void MyRepeater::Wait_RTPRecv_Event()
{
	...
	while (Mulcast_Trigger == 0){
		...
		syslog(LOG_LOCAL7 | LOG_NOTICE, "mulcastport poll  is ready\n");
		pthread_cond_wait(&mulcast_poll_cond, &poll_cond_mutex);
		//fprintf(stderr, "mulcastport poll  is running\n");
		syslog(LOG_LOCAL7 | LOG_NOTICE, "mulcastport poll  is running\n");
		gettimeofday(&start, NULL);
	}
